...
(function(){
  ...
  function validateExecutionResponse(data) {
    const required = ["accuracy_history", "loss_history", "epochs", "train_accuracy", "accuracy"];
    const missing = required.filter((key) => !(key in data));

    if (missing.length > 0) {
      throw new Error("Backend response missing metric fields: " + missing.join(", "));
    }

    if (!Array.isArray(data.accuracy_history) || !Array.isArray(data.loss_history)) {
      throw new Error("Backend metric history values must be arrays");
    }
  ...
  function updateRunStatus(status, stage = "Waiting") {
    const statusEl = $("run-status");
    const stageEl = $("run-stage");
    const executionEl = $("execution-state");
  ...
  function updateRunHistory(dataset, accuracy, loss, status) {
    runHistory.unshift({ dataset, accuracy, loss, status });

    if (runHistory.length > 3) {
      runHistory.pop();
    }

    const body = $("run-history-body");
    if (!body) return;

    body.innerHTML = runHistory.map((run) => `
      <tr>
        <td>${run.dataset}</td>
        <td>${run.accuracy}</td>
        <td>${run.loss}</td>
    ...
    if (count) count.textContent = String(runHistory.length);
  }

  function getCurrentDatasetName() {
    if (!spec.dataset) return "Unknown";

    if (spec.dataset.name) return spec.dataset.name;

    if (spec.dataset.type === "csv" && spec.dataset.path) {
      const parts = String(spec.dataset.path).split(/[\\/]/);
      return parts[parts.length - 1] || "CSV Dataset";
    }

    return spec.dataset.type || "Unknown";
  }
  ...
  function updateDatasetSummary(summary) {
    ...
    box.innerHTML = `
      <h3>Dataset Summary Preview</h3>
      <p><strong>Dataset type:</strong> ${summary.dataset_type || summary.name || "N/A"}</p>
      <p><strong>Rows used:</strong> ${summary.rows_used ?? summary.rows ?? "N/A"}</p>
      <p><strong>Label column:</strong> ${summary.label_column || "N/A"}</p>
      <p><strong>Number of features:</strong> ${summary.num_features ?? summary.feature_count ?? features.length}</p>
      <p><strong>Selected features:</strong> ${features.length ? features.join(", ") : "N/A"}</p>
      ${dropped.length ? `<p><strong>Dropped non-numeric features:</strong> ${dropped.join(", ")}</p>` : ""}
    `;
  }
  ...
  function updateTrainingSummary(metrics) {
    ...
    box.innerHTML = `
      ...
      <div class="summary-card">
        ...
        <span>${formatNumber(summary.best_train_accuracy ?? metrics.best_train_accuracy ?? metrics.train_accuracy)}</span>
      </div>
      <div class="summary-card">
        <strong>Final Loss</strong>
        <span>${formatNumber(summary.final_loss ?? metrics.final_loss ?? metrics.loss_history?.[metrics.loss_history.length - 1])}</span>
      </div>
      <div class="summary-card">
...
`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    on("btn-download-metrics", () => {
      if (!lastMetrics) {
        return setMsg("Run the pipeline first before downloading metrics.", "err");
      }
      ...
      download("training_metrics.json", JSON.stringify(exportPayload, null, 2), "application/json");
    ...
    on("btn-run", async () => {
      try {
        runCancelled = false;
        updateRunStatus("Running", "Preparing Data");

        setMsg("Preparing data and validating pipeline...");
        ...
        if (runCancelled) {
          updateRunStatus("Cancelled", "Stopped");
          setMsg("Pipeline execution cancelled.", "err");
          return;
        }

        updateRunStatus("Running", "Building Circuit");
        setMsg("Building circuit and preparing execution...");
        ...
        updateRunStatus("Running", "Training");
        setMsg("Training model...");
        ...
        updateRunStatus("Running", "Evaluating");
        setMsg("Evaluating model results...");
        ...
        updateRunStatus("Done", "Completed");

        const finalLoss = Array.isArray(data.loss_history) && data.loss_history.length > 0
          ? data.loss_history[data.loss_history.length - 1]
          : data.final_loss;

        updateRunHistory(
          getCurrentDatasetName(),
          formatNumber(data.train_accuracy || data.accuracy),
          formatNumber(finalLoss),
          "Completed"
        );

        setMsg("Pipeline finished. Dataset summary, training summary, charts, and run history updated successfully.", "ok");
      } catch (e) {
        clearCharts();
    ...
    on("btn-clear-history", () => {
      ...
      updateRunStatus("Idle", "Waiting");
      setMsg("Run history cleared.", "ok");
    });
